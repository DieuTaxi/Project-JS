...
$("#plateau").html(function () {
	// Variable contenant tout le script html
	var tableauhtml = '';

	// Creation de l'entête
	tableauhtml += '<tr><td></td>';
	for(var i = 0; i < lettre.length; i++)
	{
		tableauhtml += '<td class="caseEntete">' + lettre[i] + '</td>';
	}
	tableauhtml += '</tr>';
	...
	for (var j = 1; j <= 10; j++)
	{
		tableauhtml += '<tr><td class="caseEntete">' + j +'</td>';
		for (var k = 0; k < lettre.length; k++)
		{
			tableauhtml += '<td class="caseBataille" id="' + lettre[k] + j +'"></td>'
		}
		tableauhtml += '</tr>'
	}
	return tableauhtml;
...
$(".caseBataille").on("click", function () {
	jouerCoup($(this).attr('id'));
})
...
function majVueJeu(position,situation)
{
	...
	$("#" + position).css("background-color", couleur);
}
